...
@section('title', __('front.Our Partners'))
    ...
    <section class="brands-hero">
        ...
        <div class="container">
            <div class="brands-hero__inner">
                <h1 class="brands-hero__title">{{ __('front.Our Partners') }}</h1>
                <div class="brands-hero__line"></div>
                <p class="brands-hero__text">{{ __('front.Trusted brands and partnerships') }}</p>
            </div>
        </div>
    </section>
        ...
        <div class="container">
            <div class="brands-page-grid">
                @foreach ($brands as $brand)
                    <div class="brands-page-card">
                        <a href="{{ route('products.index', ['brand' => $brand->id]) }}" class="brands-page-card__link">
                            <div class="brands-page-card__image">
                                <img src="{{ asset('assets/admin/uploads/' . $brand->photo) }}" alt="{{ $brand->name ?? 'Brand' }}">
                            </div>
                            <div class="brands-page-card__overlay">
                                <span class="brands-page-card__name">{{ $brand->name ?? '' }}</span>
            ...
            @if($brands->isEmpty())
                <div class="brands-empty">
                    <p>{{ __('front.No brands available') }}</p>
                </div>
            @endif
        </div>
    </section>

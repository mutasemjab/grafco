<section class="contact-page" style="padding: 40px 0;">
    <div class="container">
        <h2 class="contact-title">{{ __('front.request_spare_parts') }}</h2>
        <p class="contact-lead">
            {{ __('front.parts_request_intro') }}
        </p>

        @if(session('parts_success'))
        ...
        <div class="contact-layout">
            <div class="contact-main" data-contact>
                <div class="contact-body">
                    <form class="contact-form" action="{{ route('parts-requests.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="contact-row">
                            <div class="contact-col">
                                ...
                                <input class="contact-input @error('company_name') is-invalid @enderror" type="text" name="company_name" value="{{ old('company_name') }}">
                                @error('company_name')
                                    <span class="text-danger" style="font-size: 14px; color: #dc3545;">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="contact-col">
                                <label class="contact-label">{{ __('front.equipment_model') }}</label>
                                <input class="contact-input @error('equipment_model') is-invalid @enderror" type="text" name="equipment_model" value="{{ old('equipment_model') }}" placeholder="{{ __('front.equipment_model_placeholder') }}">
                        ...
                        <div class="contact-row">
                            <div class="contact-col full">
                                <label class="contact-label">{{ __('front.parts_needed') }} *</label>
                                <textarea class="contact-input contact-textarea @error('parts_needed') is-invalid @enderror" name="parts_needed" placeholder="{{ __('front.parts_needed_placeholder') }}" required>{{ old('parts_needed') }}</textarea>
                                @error('parts_needed')
                                    <span class="text-danger" style="font-size: 14px; color: #dc3545;">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="contact-row contact-row-submit">
                            <button type="submit" class="contact-submit">
                                ...
                                <span class="contact-submit-text">{{ __('front.send_request') }}</span>
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <aside class="contact-side">
                ...
                <div class="contact-region">
                    <div class="region-item">
                        ...
                        <div dir="ltr" class="region-phone">{{ $setting->phone }}</div>
                    </div>
                    <div class="region-item">
                        <div class="region-flag">
                            <span>🇵🇸</span>

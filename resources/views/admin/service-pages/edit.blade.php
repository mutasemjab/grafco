...
@section('content')
<div class="container">
    <h3 class="mb-4">{{ __('messages.edit_service_page') }}</h3>

    <form action="{{ route('service-pages.update', $item->id) }}" method="POST" class="card p-4 shadow-sm">
        @csrf @method('PUT')

        <div class="form-group">
            <label>{{ __('messages.slug') }} <small class="text-muted">(e.g., software, appointment, parts)</small></label>
            <input type="text" name="slug" class="form-control @error('slug') is-invalid @enderror" value="{{ old('slug', $item->slug) }}" required>
            @error('slug')
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea name="subtitle_en" class="form-control @error('subtitle_en') is-invalid @enderror" rows="2">{{ old('subtitle_en', $item->subtitle_en) }}</textarea>
                    @error('subtitle_en')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea name="subtitle_ar" class="form-control @error('subtitle_ar') is-invalid @enderror" rows="2">{{ old('subtitle_ar', $item->subtitle_ar) }}</textarea>
                    @error('subtitle_ar')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
